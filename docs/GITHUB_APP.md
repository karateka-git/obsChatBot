...
3. фоновый worker получит публичные ID/login GitHub-аккаунта и доступные installation
   IDs; временный пользовательский token в SQLite не сохраняется;
4. бот сообщит в исходный чат об успехе, отсутствии доступных репозиториев,
   отказе, истечении кода или ошибке без внутренних терминов GitHub API.
...
Повторять `/github_connect` во втором канале не требуется.

Если отправить `/github_connect` повторно, бот покажет login уже подключённого
GitHub-аккаунта и спросит, нужно ли заменить его. Ответ `нет` оставляет текущее
подключение без изменений. Ответ `да` начинает новый Device Flow; только после его
успешного завершения новый GitHub-аккаунт и его installations атомарно заменяют
прежние. Подтвердить или отменить замену можно и в другом Telegram/VK-канале,
привязанном к тому же пользователю через `/link`.
...
команду нужно отправить заново после истечения короткой защиты от параллельного
запуска (не более 20 минут от начала предыдущей попытки).
