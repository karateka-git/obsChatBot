...
Команда `/register` создаёт пользователя и просит прислать обычный HTTPS URL
`github.com/owner/repository`. Необязательный `vault-path` передаётся после URL,
задаётся относительно корня repository с `/`; пустой путь означает корень.
